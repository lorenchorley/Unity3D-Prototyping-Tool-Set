...
namespace strange.extensions.command.impl {
    public class SignalCommandBinder : CommandBinder {
        ...
        protected ICommand createSignalCommand(object cmd, object data, List<Type> signalTypes) {
            Type cmdType = cmd is Type ? cmd as Type : cmd.GetType();
            injectionBinder.Bind(cmdType).To(cmd);
            ...
            if (data != null) {
                ...
                command = injectionBinder.GetInstance(cmdType) as ICommand;
            ...
            injectionBinder.Unbind(cmdType);
            return command;
        }

        override public ICommandBinding Bind<T>() {
            if (typeof(T).IsSubclassOf(typeof(StrictBaseSignal)))
                Debug.LogError("Cannot bind commands to strict signals (" + typeof(T).Name + ")");
