...
namespace strange.extensions.command.impl {
    public class CommandBinder : Binder, ICommandBinder, ITriggerable {
        ...
        new public virtual ICommandBinding Rebind<T>() {
            return GetBinding<T>() as ICommandBinding ?? Bind<T>();
        }

        new public virtual ICommandBinding Rebind(object key) {
            return GetBinding(key) as ICommandBinding ?? Bind(key);
        }
