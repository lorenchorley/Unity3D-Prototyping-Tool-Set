...
namespace strange.framework.impl {
    public class Binder : IBinder {
        ...
        virtual public IBinding Rebind<T>() {
            return GetBinding<T>() ?? Bind<T>();
        }

        virtual public IBinding Rebind(object key) {
            return GetBinding(key) ?? Bind(key);
        }
        ...
        virtual public IBinding GetBinding(object key, object name) {
            if (conflicts.Count > 0) {
                ...
                throw new BinderException("Binder \"" + binderName + "\" (" + GetType().Name + ") cannot fetch Bindings when the binder is in a conflicted state.\nConflicts: " + conflictSummary, BinderExceptionType.CONFLICT_IN_BINDER);
            }
        ...
        public void IterateAllBindings(Action<object, object, IBinding> callback) {
            //object name = (name == null) ? BindingConst.NULLOID : name;
            foreach (KeyValuePair<object, Dictionary<object, IBinding>> keyToNameBingings in bindings) {
                foreach (KeyValuePair<object, IBinding> nameToBindings in keyToNameBingings.Value) {
                    if (nameToBindings.Key.Equals(BindingConst.NULLOID)) {
                        callback.Invoke(keyToNameBingings.Key, null, nameToBindings.Value);
                    } else {
                        callback.Invoke(keyToNameBingings.Key, nameToBindings.Key, nameToBindings.Value);
                    }
                }
            }
        }
