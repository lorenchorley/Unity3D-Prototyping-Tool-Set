...
namespace System.Collections.Generic {
    ...
    [Serializable]
    public class ValueList<T> : IList<T>, System.Collections.IList, IReadOnlyList<T> {
        ...
        public ValueList<TOutput> ConvertAll<TOutput>(Converter<T, TOutput> converter) {
            ...
            ValueList<TOutput> list = new ValueList<TOutput>(_size);
            for (int i = 0; i < _size; i++) {
        ...
        public ValueList<T> GetRange(int index, int count) {
            ...
            ValueList<T> list = new ValueList<T>(count);
        ...
        internal sealed class FunctorComparer<Q> : IComparer<Q> {
        ...
        public struct RefEnumerator : IRefEnumerator<T> {
            ...
            internal RefEnumerator(ValueList<T> list) {
                this.list = list;
                index = 0;
            ...
            public bool MoveNext(ref T current) {

                ValueList<T> localList = list;
                ...
                return MoveNextRare(ref current);
            }

            private bool MoveNextRare(ref T current) {
...
//// Simple list that only takes structs and can hand back a ref to the struct values instead of a copy
//// The advantage is that it is extremely fast to iterate over
...
//    public ValueList() {
//        blockSize = System.Runtime.InteropServices.Marshal.SizeOf<T>() * 4092;
//        // Need to make blocksize a large multiple of 2 and SizeOf<T>()?
//        Clear();
//    }
...
//    // Note that these are the only methods that return the values by reference
//    public void Get(int index, ref T value) {
//        value = s[index];
//    }

//    //public bool TryGetValue(int index, ref T value) {
//    //    if (index < 0 || index >= count || availableIndices.Contains(index))
//    //        return false;
...
//        indices = new List<int>(blockSize);
//    }
...
//    public IRefEnumerator<T> GetEnumerator() {
//        return new RefEnumerator<T>(s, count);
//    }

//    class RefEnumerator<S> : IRefEnumerator<S> {

//        private S[] s;
//        private int index;
//        private int count;

//        public RefEnumerator(S[] s, int count) {
//            this.s = s;
//            this.count = count;
//        }
...
//        public bool MoveNext(ref S current) {
//            current = s[index]; 
//            return index++ < count;
//        }
